...
public:
  ...
  bool addQuizSet(const std::string &name,
    ...
    for (const auto &q : questions) {
      const char *insertQ = "INSERT INTO questions(quiz_set_id, text, "
                            "order_index) VALUES(?,?,?);";
  ...
  std::vector<QuizSet> getAllQuizSets() {
    ...
    const char *sql = "SELECT qs.id, qs.name, qs.created_at,"
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK) {
      while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        const char *cntSql =
            "SELECT COUNT(*) FROM questions WHERE quiz_set_id=?;";
  ...
  QuizSet getFullQuizSet(int id) {
    ...
    const char *getQ = "SELECT id, text, order_index FROM questions WHERE "
                       "quiz_set_id=? ORDER BY order_index;";
  ...
  bool deleteQuizSet(int id) {
    std::lock_guard<std::mutex> lock(dbMutex);
    std::string sql =
        "DELETE FROM quiz_sets WHERE id=" + std::to_string(id) + ";";
    return execSQL(sql);
  }
  ...
  bool saveResult(const QuizResult &result) {
    std::lock_guard<std::mutex> lock(dbMutex);
    const char *sql = "INSERT INTO results(quiz_set_id, score, total, "
                      "percentage) VALUES(?,?,?,?);";
    sqlite3_stmt *stmt = nullptr;
...
private:
  ...
  void createIndexes() {
    execSQL("CREATE INDEX IF NOT EXISTS idx_questions_set ON "
            "questions(quiz_set_id);");
    execSQL(
        "CREATE INDEX IF NOT EXISTS idx_answers_q ON answers(question_id);");
    execSQL("CREATE INDEX IF NOT EXISTS idx_results_pct ON results(percentage "
            "DESC);");
  }
...
public:
  ...
  static ParseResult parse(const std::string &filename) {
    ...
    if (slash == std::string::npos)
      ...
      ++slash;
    result.suggestedName = (dot != std::string::npos && dot > slash)
    ...
    auto saveQuestion = [&]() {
      if (!currentQuestion.empty() && !currentAnswers.empty()) {
        ...
        if (!hasCorrect) {
          result.errors.push_back("Savol '" + currentQuestion.substr(0, 30) +
                                  "...' to'g'ri javobsiz");
        } else {
          Question q;
...
public:
  ...
  const Question &getCurrentQuestion() const {
    if (!hasMore())
      throw std::out_of_range("Savollar tugadi");
  ...
  bool submitAnswer(int answerIndex) {
    if (answerIndex < 0 ||
        answerIndex >= static_cast<int>(currentAnswers.size()))
      return false;
...
private:
  ...
  Gtk::Button btnHome, btnNew, btnImport, btnPlay, btnDelete, btnSettings,
      btnQuit;
  ...
  static Gtk::Button *makeButton(const std::string &label,
...
inline MainWindow::MainWindow() : db() {
...
inline void MainWindow::applyCSS(bool dark) {
  ...
  std::string palette =
      dark ? "window { background: #1e1e2e; color: #cdd6f4; }"
             ".card { background: #313244; border-radius: 12px; padding: 16px; "
             "margin: 8px; }"
             "entry { background: #313244; color: #cdd6f4; border: 2px solid "
             "#45475a; border-radius: 8px; padding: 8px; }"
             "entry:focus { border-color: #cba6f7; }"
             "textview { background: #313244; color: #cdd6f4; }"
             "list { background: #313244; border-radius: 8px; }"
             "list row { padding: 10px; border-bottom: 1px solid #45475a; "
             "color: #cdd6f4; }"
           ...
           : "window { background: #f5f5f5; color: #222; }"
             ...
             "margin: 8px; }"
             "entry { background: white; color: #222; border: 2px solid "
             "#e0e0e0; border-radius: 8px; padding: 8px; }"
...
inline Gtk::Button *MainWindow::makeButton(const std::string &label,
                                           const std::string &cssClass) {
...
// Navigation
inline void MainWindow::goHome() {
  refreshHome();
  mainStack.set_visible_child("home");
}
inline void MainWindow::goNew() {
  clearNewQuiz();
  mainStack.set_visible_child("new");
}
inline void MainWindow::goImport() { mainStack.set_visible_child("import"); }
inline void MainWindow::goPlay() {
  refreshPlayList();
  mainStack.set_visible_child("play");
}
inline void MainWindow::goDelete() {
  refreshDeleteList();
  mainStack.set_visible_child("delete");
}
inline void MainWindow::goSettings() {
  mainStack.set_visible_child("settings");
}
...
inline void MainWindow::buildUI() {
  ...
  btnImport.set_label("📂");
  btnPlay.set_label("🎮");
  btnDelete.set_label("🗑️");
  btnSettings.set_label("⚙️");
  btnQuit.set_label("❌");
  ...
  btnImport.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::goImport));
  btnPlay.signal_clicked().connect(sigc::mem_fun(*this, &MainWindow::goPlay));
  btnDelete.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::goDelete));
  btnSettings.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::goSettings));
...
inline Gtk::Widget *MainWindow::buildHomePage() {
  ...
  auto *title = makeLabel("<span size='xx-large' weight='bold' "
                          "foreground='#667eea'>📚 QUIZ MASTER PRO</span>");
  title->set_halign(Gtk::ALIGN_CENTER);
  grid->attach(*title, 0, 0, 2, 1);

  auto *sub = makeLabel("<span size='large' foreground='#888'>\"Bilimingizni "
                        "sinovdan o'tkazing\"</span>");
  ...
  lblStats->set_markup(
      "<span foreground='white' size='large'>📊 Yuklanmoqda...</span>");
  ...
  for (auto &c : cards) {
    ...
    auto *tl =
        makeLabel("<span weight='bold' size='large'>" + c.title + "</span>");
  ...
  // Top results
  auto *resTitle =
      makeLabel("<span weight='bold' size='large'>🏆 Top Natijalar</span>");
...
inline Gtk::Widget *MainWindow::buildNewPage() {
  ...
  vb->pack_start(
      *makeLabel("<span class='page-title' size='x-large' weight='bold' "
                 "foreground='#667eea'>✨ Yangi Quiz Yaratish</span>"),
      Gtk::PACK_SHRINK);
  ...
  vb->pack_start(
      *makeLabel(
          "<b>Javoblar (kamida 2 ta, bittasini ✔ to'g'ri deb belgilang):</b>"),
      Gtk::PACK_SHRINK);
  ...
  addAnsBtn->signal_clicked().connect([this]() {
    addAnswerRow();
    show_all_children();
  });
  ...
  addQBtn->signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onSaveQuestion));
  ...
  saveBtn->signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onSaveQuiz));
...
inline void MainWindow::onSaveQuestion() {
  ...
  if (qText.empty()) {
    auto *dlg =
        new Gtk::MessageDialog(*this, "Savol matnini kiriting!", false,
  ...
  if (q.answers.size() < 2) {
    auto *dlg =
        new Gtk::MessageDialog(*this, "Kamida 2 ta javob kiriting!", false,
                               Gtk::MESSAGE_WARNING, Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
  ...
  if (!hasCorrect) {
    auto *dlg = new Gtk::MessageDialog(
        *this, "Kamida 1 ta to'g'ri javobni belgilang!", false,
        Gtk::MESSAGE_WARNING, Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
  ...
  auto *row = Gtk::make_managed<Gtk::ListBoxRow>();
  auto *lbl =
      makeLabel("<b>" + std::to_string(nqs.questions.size()) + ".</b> " +
                qText.substr(0, 60) + (qText.size() > 60 ? "..." : ""));
  ...
  lblQCount->set_markup(
      "<b>Savollar soni: " + std::to_string(nqs.questions.size()) + "</b>");
  ...
  for (auto *r : nqs.answerRows) {
    ...
    if (auto *ent = dynamic_cast<Gtk::Entry *>(ch.size() > 1 ? ch[1] : nullptr))
      ent->set_text("");
    if (auto *chk =
            dynamic_cast<Gtk::CheckButton *>(ch.size() > 0 ? ch[0] : nullptr))
...
inline void MainWindow::onSaveQuiz() {
  ...
  if (name.empty()) {
    auto *dlg =
        new Gtk::MessageDialog(*this, "Quiz nomini kiriting!", false,
                               Gtk::MESSAGE_WARNING, Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
  ...
  if (nqs.questions.empty()) {
    auto *dlg = new Gtk::MessageDialog(
        *this, "Hech bo'lmasa 1 ta savol qo'shing!", false,
        Gtk::MESSAGE_WARNING, Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
  ...
  if (db.addQuizSet(name, nqs.questions)) {
    auto *dlg = new Gtk::MessageDialog(
        *this,
        "✅ Quiz saqlandi! (" + std::to_string(nqs.questions.size()) +
            " savol)",
    ...
    auto *dlg =
        new Gtk::MessageDialog(*this, "❌ Saqlashda xato!", false,
...
inline void MainWindow::clearNewQuiz() {
  ...
  if (lblQCount)
    lblQCount->set_markup("<b>Savollar soni: 0</b>");
  if (lbQuestionList)
...
inline Gtk::Widget *MainWindow::buildImportPage() {
  ...
  vb->pack_start(
      *makeLabel("<span size='x-large' weight='bold' foreground='#667eea'>📂 "
                 "TXT Fayldan Import</span>"),
      Gtk::PACK_SHRINK);
  ...
  chooseBtn->signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onChooseFile));
  ...
  btnDoImport->signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onDoImport));
...
inline void MainWindow::onChooseFile() {
  Gtk::FileChooserDialog dlg("TXT faylni tanlang",
  ...
  if (dlg.run() == Gtk::RESPONSE_OK) {
    ...
    if (lastParseResult.success) {
      ...
      if (!lastParseResult.errors.empty())
        msg += " (⚠️ " + std::to_string(lastParseResult.errors.size()) +
               " ogohlantirish)";
      ...
      std::string errs;
      for (auto &e : lastParseResult.errors)
        errs += "• " + e + "\n";
      lblParseResult->set_markup("<span foreground='red'>❌ Xato:\n" + errs +
                                 "</span>");
      btnDoImport->set_sensitive(false);
    }
  }
...
inline void MainWindow::onDoImport() {
  ...
  if (name.empty())
    name = lastParseResult.suggestedName;
  ...
  if (db.addQuizSet(name, lastParseResult.questions)) {
    ...
    auto *dlg =
        new Gtk::MessageDialog(*this, "❌ Import xato!", false,
                               Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
...
inline Gtk::Widget *MainWindow::buildPlayPage() {
  ...
  vb->pack_start(*makeLabel("<span size='x-large' weight='bold' "
                            "foreground='#667eea'>🎮 Quiz O'yna</span>"),
                 Gtk::PACK_SHRINK);
  ...
  startBtn->signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::startQuiz));
  ...
  btnNextQ->signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onNextQuestion));
...
inline void MainWindow::refreshPlayList() {
  ...
  for (auto &s : sets) {
    ...
    auto *lbl =
        makeLabel("🗂️ <b>" + s.name + "</b>  <span foreground='#888'>(" +
...
inline void MainWindow::startQuiz() {
  ...
  if (!row) {
    auto *dlg =
        new Gtk::MessageDialog(*this, "Quiz setini tanlang!", false,
                               Gtk::MESSAGE_WARNING, Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
  ...
  if (quizSet.questions.empty()) {
    auto *dlg =
        new Gtk::MessageDialog(*this, "Bu setda savollar yo'q!", false,
                               Gtk::MESSAGE_WARNING, Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
...
inline void MainWindow::showQuestion() {
  if (!engine.hasMore()) {
    ...
    auto *dlg = new Gtk::MessageDialog(*this, msg, false, Gtk::MESSAGE_INFO,
                                       Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
  ...
  lblQNum->set_markup("<span foreground='#667eea'><b>Savol " +
  ...
  lblQuestion->set_markup("<span size='large' weight='bold'>" + q.text +
                          "</span>");
  ...
  lblScore->set_markup("⭐ Ball: <b>" + std::to_string(engine.getScore()) +
...
inline void MainWindow::onAnswerClicked(int idx) {
  ...
  auto answers =
      engine.getShuffledAnswers(); // re-read current (already stored)
...
inline Gtk::Widget *MainWindow::buildDeletePage() {
  ...
  vb->pack_start(
      *makeLabel("<span size='x-large' weight='bold' foreground='#e74c3c'>🗑️ "
                 "Quiz Setni O'chirish</span>"),
      Gtk::PACK_SHRINK);

  vb->pack_start(*makeLabel("<b>O'chirish uchun setni tanlang:</b>"),
                 Gtk::PACK_SHRINK);
  ...
  refreshBtn->signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::refreshDeleteList));

  auto *delBtn = makeButton("🗑️ O'chirish", "btn-danger");
  delBtn->signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onDeleteSelected));

  hb->pack_start(*refreshBtn, Gtk::PACK_SHRINK);
  hb->pack_start(*delBtn, Gtk::PACK_SHRINK);
  vb->pack_start(*hb, Gtk::PACK_SHRINK);

  auto *warn = makeLabel("<span foreground='#e74c3c'>⚠️ Diqqat! O'chirilgan "
                         "setni qayta tiklab bo'lmaydi!</span>",
                         true);
...
inline void MainWindow::refreshDeleteList() {
  ...
  for (auto &s : sets) {
    ...
    row->set_data("quiz_id", GINT_TO_POINTER(s.id));
    auto *lbl =
        makeLabel("📋 <b>" + s.name + "</b>  <span foreground='#888'>(" +
                  std::to_string(s.questions.size()) + " savol)</span>");
    lbl->set_margin_top(10);
    lbl->set_margin_bottom(10);
    lbl->set_margin_start(10);
...
inline void MainWindow::onDeleteSelected() {
  ...
  if (!row) {
    auto *dlg =
        new Gtk::MessageDialog(*this, "O'chirish uchun set tanlang!", false,
                               Gtk::MESSAGE_WARNING, Gtk::BUTTONS_OK, true);
    dlg->run();
    dlg->hide();
    delete dlg;
  ...
  auto *dlg = new Gtk::MessageDialog(
      *this, "Haqiqatan ham o'chirishni xohlaysizmi?", false,
  ...
  if (resp == Gtk::RESPONSE_YES) {
    if (db.deleteQuizSet(id)) {
      ...
      auto *err =
          new Gtk::MessageDialog(*this, "❌ O'chirishda xato!", false,
                                 Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK, true);
      err->run();
      err->hide();
      delete err;
...
inline Gtk::Widget *MainWindow::buildSettingsPage() {
  ...
  vb->pack_start(*makeLabel("<span size='x-large' weight='bold' "
                            "foreground='#667eea'>⚙️ Sozlamalar</span>"),
                 Gtk::PACK_SHRINK);
  ...
  dmRow->pack_start(*makeLabel("Qorong'i rejim (Dark Mode):"),
                    Gtk::PACK_SHRINK);
  swDarkMode = Gtk::make_managed<Gtk::Switch>();
  swDarkMode->signal_state_set().connect(
  ...
  aboutBox->pack_start(*makeLabel("<b>Quiz Master Pro</b> v1.0.0"),
                       Gtk::PACK_SHRINK);
  aboutBox->pack_start(*makeLabel("C++17 + GTKMM3 + SQLite3"),
                       Gtk::PACK_SHRINK);
